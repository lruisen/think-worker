...
class Monitor
{
	...
	{
		static::resume();
		if (! Worker::getAllWorkers() || empty($this->options)) {
			return;
		}
		...
		if (in_array('exec', $disableFunctions, true)) {
			...
			Timer::add($this->options['interval'] ?? 2, [$this, 'checkAllFilesChange']);
		}

		$memoryLimit = $this->getMemoryLimit($this->options['memory_limit'] ?? null);
		if ($memoryLimit && DIRECTORY_SEPARATOR === '/') {
			Timer::add($this->options['memory_monitor_interval'] ?? 60, [$this, 'checkMemory'], [$memoryLimit]);
		}
	}
